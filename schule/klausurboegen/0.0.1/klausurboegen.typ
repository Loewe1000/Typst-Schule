...
#let klausurbögen(exam: "", subexam:"", teacher: "SLZ", class: "PH1", date: "09.10.2023", students: (), sek1: false, result: false, rand: 5cm, sub: false, numbering: "a)", weißer-rand: true) = [
  ...
  #let klausurbogen(result: false, name:"", note-content:[], student-table: [], grade-table:[], sub: sub) = [
  ...
  #let heigth = if sub {6cm} else {8cm}
  #table(stroke: none, inset: 0cm, rows: 1fr, columns: 2 * (1fr,),
  [
  #if not weißer-rand and not result {
    place(line(stroke: 2pt, start:(real-rand, 0%), end: (real-rand, 100%))  )
  }
  #table(stroke: none, inset: 0cm, rows: 1fr, columns: (rand, 1fr),
      [
        #if not result and exam != "" {place(bottom+left, [#box(inset: insetPageNumber, fill: white,[#text(size:16pt,[*4*])])])}
      ],
      [
  ...
  [
    #if not weißer-rand and not result {
      place(line(stroke: 2pt, start:(real-rand, 0% + heigth + 5mm), end: (real-rand, 100%))  )
    }
    #if exam != "" [
    #box(inset:(top: 7mm, left: 4mm, right: 10mm), height: heigth, [
          #if result {
    ...
    #if not result and exam != "" {place(bottom+right, [#box(inset: insetPageNumber, fill: white,[#text(size:16pt,[*1*])])])}
    #table(stroke: none /*0.5pt+rgb(129,129,129)*/, inset: 0cm, rows: 1fr, columns: (1fr, rand),
      [
        #if not result {place(kariert(height: 30cm - if sub {6cm} else {8cm} - 0.5cm))}
      ],
    )
  ]
  )
  ...
  #table(stroke: none, inset: 0cm, rows: 1fr, columns: (rand, 1fr),
      [
        #if exam != "" [#place(bottom+left, [#box(inset: insetPageNumber, fill: white,[#text(size:16pt,[*2*])])])]
      ],
      [
  ...
  [
    ...
    #table(stroke: none /*0.5pt+rgb(129,129,129)*/, inset: 0cm, rows: 1fr, columns: (1fr, rand),
      ...
      [
        #if exam != "" [#place(bottom+right, [#box(inset: insetPageNumber, fill: white,[#text(size:16pt,[*3*])])])]
      ]
    )
  ]
  )
  ...
  #let gradeBoundaries(maxPoints: 100) = {
    ...
    for (index, value) in gradeBorders.enumerate() {
      ...
      if sek1 {
        ...
        seperationLines = (1,4,7,10,13,16).map(x => vlinex(x: x, stroke: if result {black.lighten(50%)} else {white} + 2pt))
      }
    }
    ...
    tablex(
      ...
      inset: 6pt,
      stroke: if result {print-color} else {print-color} + 1pt,
      align: center + horizon,
      ..seperationLines,
      [*Note*], ..grades,
      [#if sek1 [*Punktegrenzen*] else [*Punktegrenze $>=$*] ], ..if result { gradeRegions}
    )
    }
  }
  ...
  #let studentHeader(name: "Max Mustermann", percentage: 0.8462, note: 6) = {
    grid(
      gutter: 10pt,
      [#text(12pt)[#name]],
      [#grid(columns: (auto, 1fr, auto), text(12pt)[#exam], [], text(12pt, gray, weight: "semibold")[#date])
  ...
  #let createTable(cols: none, maxPoints: none, achievedPoints: none, result: false) = {
    ...
    if not subtasks {
      ...
      if c == none {
        ...
        (..c, fill: if col > numberOfColumns { if result { print-color.lighten(90%) } else { print-color} })
      }
    ),
      rowspanx(2)[*Aufgabe*], ..headerRow, rowspanx(2)[$sum$],
      ..subtaskRow.map(x => [*#x*]),
      text(size: 8pt,[*mögliche Punkte*]), ..maxPoints.map(x => if result {cellx(inset: 0mm, text(size:10pt, fill: black,str(x).replace(".", ",")))}), if result {cellx(inset: 0mm, text(size:10pt, fill: black,str(maxPoints.sum()).replace(".", ",")))},
      text(size: 8pt,[*erreichte Punkte*]), ..if result {achievedPoints.map(x => cellx(inset: 0mm, text(size:10pt, fill: black, str(x).replace(".", ","))))}, if result {cellx(inset: 0mm, text(size:10pt, fill: black, str(achievedPoints.sum()).replace(".", ",")))}
    )
  ...
  #{
    ...
    if exam != "" {
    if students.len() == 0 {
      klausurbogen(result: result, name: "", note-content: [],
        student-table: createTable(cols: cols, maxPoints: 1, achievedPoints: 1, result: result),
        grade-table: gradeBoundaries(maxPoints: 1))
    } else {
    for (key, value) in students.enumerate() {
      if (key == 0) {
        ...
        klausurbogen(result: result, name: name, note-content: note-display(percentage: points.sum()/maxPoints.sum(), note: note),
