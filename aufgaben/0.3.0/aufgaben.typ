...
/// Setzt alle Aufgaben-Zähler und -Zustände zurück.
///
/// Notwendig für den Bundle-Export (Typst 0.15+): Dort sind States und
/// Counter global über alle Dokumente des Bundles. Damit jede Variante
/// (Druck, Digital, Lösung) wieder bei Aufgabe 1 beginnt und sich die
/// registrierten Aufgaben/Lösungen/Erwartungen nicht über die Varianten
/// hinweg ansammeln, wird diese Funktion am Anfang jedes Varianten-Dokuments
/// aufgerufen. In Einzeldokumenten ist der Aufruf wirkungslos, aber harmlos.
///
/// ```typ
/// #reset-aufgaben()
/// ```
...
/// -> content
#let reset-aufgaben() = {
  _counter_aufgaben.update((0, 0))
  counter(figure.where(kind: "aufgabe")).update(0)
  counter(figure.where(kind: "teilaufgabe")).update(0)
  counter(figure.where(kind: "material")).update(0)
  counter(figure.where(kind: image)).update(0)
  counter(figure.where(kind: table)).update(0)
  _state_aufgaben.update(())
  _state_current_aufgabe_index.update(-1)
  _state_current_material_aufgabe.update(0)
  _state_current_material_index.update(0)
  _state_in_loesung.update(false)
  _state_loesungen_gezeigt.update(0)
}
...
/// Gibt im Lösungsmodus `"nur"` alle noch nicht angezeigten Lösungen aus.
///
/// Idempotent: Jede Aufgabe wird höchstens einmal ausgegeben (verfolgt über
/// einen internen Zähler-State). Wird automatisch beim Beginn jeder neuen
/// Aufgabe sowie am Dokumentende aufgerufen; Templates wie `klassenarbeit`
/// können die Ausgabe damit auch früher anstoßen (z. B. vor dem
/// Erwartungshorizont).
///
/// -> content
#let show-offene-loesungen() = {
  context {
    let alle = _state_aufgaben.get()
    let gezeigt = _state_loesungen_gezeigt.get()
    for i in range(gezeigt, alle.len()) {
      show-loesung-inline(alle.at(i))
    }
  }
  context _state_loesungen_gezeigt.update(_state_aufgaben.get().len())
}
...
#let aufgabe(
  ...
  // "nur"-Modus: Lösungen der VORHERIGEN Aufgaben anzeigen, bevor die neue
  // beginnt. So sind auch Lösungen erfasst, die nach dem aufgabe()-Aufruf
  // registriert wurden (z. B. von Shortcode-Teilaufgaben außerhalb des
  // Bodys). Die letzte Aufgabe wird vom Template am Dokumentende ausgegeben.
  context if _state_options.get().loesungen == "nur" {
  ...
  // "sofort" materials
  context if _state_options.get().materialien in ("sofort", "seiten", "folgend") and _state_aufgaben.get().last().materialien.len() > 0 and _state_options.get().loesungen != "nur" {
    if _state_options.get().materialien == "seiten" {
      pagebreak(weak: true)
    }
  ...
  // "nur"-Modus: Die Lösungen dieser Aufgabe werden erst beim Beginn der
  // nächsten Aufgabe bzw. am Dokumentende angezeigt (siehe oben), damit auch
  // nachträglich registrierte Lösungen von Teilaufgaben erfasst sind.
...
#let teilaufgabe(
  ...
  context {
    ...
    // "nur"-Modus: Teilaufgabe unsichtbar layouten, damit loesung()/
    // erwartung() im Body trotzdem registriert werden (analog zu aufgabe()).
    if opts.loesungen == "nur" {
      place(hide(ta-fig))
    } else {
      ta-fig
    }
