...
#let marginnote(pos: left, gutter: .5em, dy: 0em, body) = {
  style(styles => {
...
#let __foreach(s, func, filter: elem => true, final: true, loc: none) = {
  assert(type(s) == "state")
...
// %     Display Funktionen       %
// %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
#let d_aufg(label: "Aufgabe", format: "1", nr: none) = [#label
  #if nr == none {
    __c_aufgaben.display((..c) => numbering(format, c.pos().first()))
  } else {
    numbering(format, nr)
  }
]
#let d_punkte(punkte, singular: "Punkt", plural: "Punkte") = {
...
#let d_erwpunkte(
  aufg: none,
  teil: none,
  format: punkte => [#align(right)[_(#punkte.join(" + "))_]],
) = {
  locate(loc => {
    let n = aufg
    if aufg == none {
...
#let d_loesung() = {
  __akt_aufg(aufg => block(inset: 0.5em, fill: luma(240), radius: 4pt)[
    === Lösungen #d_aufg()
    #__d_loesung(aufg)
  ])
}
...
#let loesung(aufg: none, teil: none, body) = {
  locate(loc => {
    ...
    __update_aufg(nr: aufg, a => {
      a.loesung.push((teil: t, body: body))
      a
    })
  })
  options.get(
    ...
    value => {
      if value == "sofort" {
        block(width: 100%, inset: 0.5em, fill: luma(240), radius: 4pt)[=== Lösung
          #body]
      }
    },
  )
...
#let erwartung(text, punkte, aufg: none, teil: none) = {
  locate(loc => {
    let n = aufg
    if n == none {
      n = __akt_aufgnr(v => v, loc: loc)
    ...
    __s_erwartungen.update(e => {
      e.push((aufgabe: n, teil: t, text: text, punkte: punkte))
      e
    })
  })
}