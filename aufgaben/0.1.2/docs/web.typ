...
#show: it => template(
  ...
  license: "MIT",
  repository: "https://github.com/Loewe1000/Typst-Schule",
  universe: "https://typst.app/universe/package/aufgaben",
  notices: (
...
+ *Installation und Import* -- Erste Schritte
...
#table(
  columns: (auto, auto, 1fr),
  [*Parameter*], [*Standard*], [*Beschreibung*],
  [`title`], [`none`], [Titel der Aufgabe (auch als 1. Positionsargument möglich)],
  [`method`], [`""`], [Sozialform: `"EA"`, `"PA"`, `"GA"`],
  [`icons`], [`()`], [Array mit zusätzlichen Icons],
  [`large`], [`true`], [Größere Überschrift (1.25em statt 1em)],
  [`number`], [`true`], [Aufgabennummer anzeigen],
  [`workspace`], [`none`], [Arbeitsbereich (z. B. `v(3cm)`)],
  [`label`], [`none`], [Label zur Referenzierung],
  [`body`], [--], [Inhalt der Aufgabe],
)
...
#table(
  columns: (auto, auto, 1fr),
  [*Parameter*], [*Standard*], [*Beschreibung*],
  [`item-label`], [`none`], [Benutzerdefinierte Beschriftung statt auto. Nummerierung],
  [`label`], [`none`], [Label zur Referenzierung],
  [`workspace`], [`none`], [Arbeitsbereich für diese Teilaufgabe],
  [`body`], [--], [Inhalt der Teilaufgabe],
)
...
Definiert eine Erwartung mit Punktzahl.

```typ
#erwartung(punkte)[beschreibung]
```

`punkte` kann weggelassen werden (dann 0). Erwartungen werden automatisch der aktuellen Aufgabe oder Teilaufgabe zugeordnet.
...
#table(
  columns: (auto, auto, 1fr),
  [*Parameter*], [*Standard*], [*Beschreibung*],
  [`body`], [--], [Inhalt des Materials],
  [`caption`], [`none`], [Beschriftung],
  [`label`], [`none`], [Label zur Referenzierung],
)
...
#table(
  columns: (auto, auto, 1fr),
  [*Parameter*], [*Standard*], [*Beschreibung*],
  [`curr`], [`false`], [Nur Lösung der letzten Aufgabe anzeigen],
  [`teil`], [`false`], [Nur Lösungen einer bestimmten Teilaufgabe (Zahl)],
)
...
#table(
  columns: (auto, auto, 1fr),
  [*Parameter*], [*Standard*], [*Beschreibung*],
  [`curr`], [`false`], [Nur Material der letzten Aufgabe anzeigen],
)
...
#table(
  columns: (auto, auto, 1fr),
  [*Parameter*], [*Standard*], [*Beschreibung*],
  [`grouped`], [`false`], [Erwartungen pro Teilaufgabe gruppieren],
  [`new-page`], [`false`], [Auf neuer Seite beginnen],
  [`erreicht`], [`false`], [Format `__ / X` für eintragbare Punkte],
)

== `show-bewertung()`

Zeigt eine Bewertungstabelle an. Parameter: `true` (mit möglichen Punkten), `false` (leere Felder), `none` (nur erreichte-Punkte-Zeile).

== `set-options()`

Setzt Konfigurationsoptionen als Dictionary:

```typ
#set-options((option: wert))
```

== `get-points()`

Gibt die Punktzahl einer Aufgabe zurück.

```typ
#context get-points(aufgabe-nr)
#context get-points(aufgabe-nr, teil: teilaufgabe-nr)
```
