...
#let klausurbögen(exam: "", subexam:"", teacher: "SLZ", class: "PH1", date: "09.10.2023", students: (), sek1: false, result: false, rand: 5cm, sub: false, numbering: "a)", weißer-rand: true, result-table: true) = [
  ...
  #let heigth = if sub and result-table {6cm} else if sub {3cm} else {8cm}
  ...
  [
    #table(stroke: none, inset: 0cm, rows: 1fr, columns: (rand, 1fr),
      ...
      place(top+left,line(stroke: 1pt+dark-red, start:(real-rand, 0%), end: (real-rand, 100%))  )
    }
  ],
  [
    #if exam != "" [
    ...
    #if not weißer-rand and not result {
      place(top+right,line(stroke: 1pt+dark-red, start:(-real-rand, 0% + heigth + 5mm), end: (-real-rand, 100%))  )
    }
  ]
  )
  #if not result [
  ...
  [
    ...
    #if not weißer-rand and not result {
      place(top+right,line(stroke: 1pt+dark-red, start:(-real-rand, 0%), end: (-real-rand, 100%))  )
    }
  ]
  )
  ]
