#import "@schule/arbeitsblatt:0.1.1": *
...
#let klassenarbeit(title: "", subtitle: "", class: "", date: "", logo: "", teacher: "", font-size:12pt, table:() , stufe: false, loesungen: none, erwartungen: false, bewertung: false, page-numbering: true, klausurboegen: false, ergebnisse: none, klausurboegen-settings:(sub: false, numbering: "a)", rand: 6cm, weißer-rand: true,), ..args, body) = {
...
tablex(
    ...
    range(0, table.len()).map( key => (
      (cellx(inset:(left:0pt, top:6pt, bottom: 6pt, right: 6pt),[*#{table.at(key).at(0)}:*]), [#{table.at(key).at(1)}])
    )).flatten()
    },
    hlinex(),
  )
