...
#let __d__erwartung(text, punkte, last) = {
  if last [#text (#punkte)] else [#text (#punkte)\ ]
}
...
#let d_ew_text = d_ew_erwartungen.with(
  format: (t, p, last) => {
    if last [#sym.dots #t] else [#sym.dots #t\ ]
  },
)
...
#let d_ew_gesamt(format: p => [#p]) = locate(loc => {
  format(__s_erwartungen.final(loc).fold(0, (p, erw) => p + erw.punkte))
})
...
#let d_ew_notenspiegel(
  ...
  locate(loc => {
    ...
    let punkte = __s_erwartungen.final(loc).fold(0, (p, erw) => p + erw.punkte)
...
#let d_ew_unterstufe() = [
  ...
  #locate(loc => {
    ...
    tablex(
      ...
      inset: 7pt,
      align: (col, row) => {
        if col in (0, 2, 3) { center + horizon } else if row == 0 {
          horizon
        } else { left }
      },
      cellx(fill: cell-highlight-color)[*Aufg.*],
      cellx(fill: cell-highlight-color)[Die Schülerin / Der Schüler #sym.dots.h],
      cellx(fill: cell-highlight-color)[*mögl. \ Punkte*],
      .._ew_cells(),
      cellx(fill: cell-highlight-color)[],
      cellx(fill: cell-highlight-color)[
        #set align(right)
        *Insgesamt*:
      ],
      ...
      __s_aufgaben
        ...
        .map(aufg => {
          let cells = (
            ...
            cellx(fill: cell-highlight-color)[_#{aufg.title}_],
